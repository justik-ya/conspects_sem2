...
\begin{proof}
    ...
    Доказательство:
    \[
    \left(\int^b_a |f(x)|^p dx \right)^{\frac{1}{p}} = 0 \Leftrightarrow 
    \int^b_a |f(x)|^p dx = 0 \Leftrightarrow 
    \]
    \[
    \Leftrightarrow |f(x)|^p = 0 \Leftrightarrow
    f(x) = 0 \ \ \text{(за исключением конечного числа точек)}
    \]
    \[
    ...
    $f(x) = 0$ (за исключением конечного числа точек) $\Rightarrow \int^b_a |f(x)| dx = 0 \Rightarrow$

    \[ 0 \le \int^d_c |f(x)g(x)| dx \le M(c, d)\cdot 0 = 0 \]
...
\begin{proof}
    \[
    \int^b_a \sqrt{\sum^d_{k=1}\ \ (\vphi'(t))^2}\ \ dt\ \ \overset{(!)}{\le} \qquad l(\Gamma) \qquad \overset{(!)}{\le} \ \int^b_a \sqrt{\sum^d_{k=1}\ \ (\vphi'(t))^2}\ \ dt
    \]
