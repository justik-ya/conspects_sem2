...
\subsection{Разрешение коллизий линейным зондированием}
...
\begin{algorithmic}[1]
	...
	\If{$2n > m$}
	\State Выделяем новую таблицу размера $2m$
	\State Вставляем в неё все ключи из $T$
	\State Удаляем $T$ (освобождаем память)
	\State $m = 2m$
	\EndIf
\end{algorithmic}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item $q_k$ — вероятность того, что в таблице существует серия длины $k$, начинающаяся в позиции $t - (k - \ell)$ (эта вероятность не зависит от конкретной позиции $t$);
		\item $p_\ell$ — вероятность встретить в позиции $t = h(x)$ «хвост» длины $\ell$ некоторой серии длины $k \geq \ell$:
		\begin{align*}
			p_\ell = \sum_{k=\ell}^{n} q_k.
		\end{align*}
	\end{itemize}
	
	\noindent\textbf{Ожидаемое время.} Оно равно $O\!\left(\displaystyle\sum_{\ell=0}^{n} p_\ell \cdot \ell\right)$. Преобразуем:
	\begin{align*}
	...
	\noindent\textbf{Оценка $q_k$.} Докажем, что $q_k \leq \dfrac{1}{e^{\Omega(k)}}$. Из этого будет следовать:
	\begin{align*}
		\sum_{k=0}^{n} q_k \cdot \Theta(k^2)
		\leq \Theta\!\left(\sum_{k=0}^{n} \frac{k^2}{e^{\Omega(k)}}\right) = O(1).
	\end{align*}
	
	Если в таблице есть серия в позициях $t', t'+1, \ldots, t'+k-1$, значит, ровно $k$ ключей отображены в ячейки $\{t', t'+1, \ldots, t'+k-1\}$, а $n - k$ ключей — в ячейки, не равные этим.
	
	Следовательно ($m = 2n$):
	\begin{align*}
		q_k
		&\leq \binom{n}{k} \left(\frac{k}{2n}\right)^{\!k} \left(1 - \frac{k}{2n}\right)^{\!n-k}.
	\end{align*}
	
	Оценим сверху каждый множитель. Используем неравенство $1 + x \leq e^x$:
	
	\begin{align*}
		q_k
		&\leq \frac{n^{n-k} \cdot k^k}{k!\,(n-k)! \cdot n^k} \cdot \frac{k^k}{2^k \cdot n^k} \cdot e^{-\frac{(n-k)k}{2n}} \\[4pt]
		&= \frac{1}{2^k} \cdot \left(\frac{n}{n-k}\right)^{\!n-k} \cdot e^{-(\cdots)} \\[4pt]
		&= \frac{1}{2^k} \cdot \left(1 + \frac{k}{n-k}\right)^{\!n-k} \cdot e^{-(\cdots)} \\[4pt]
		&\leq \frac{1}{2^k} \cdot e^{k} \cdot e^{-\frac{k}{2n}(n-k)} \\[4pt]
		&= \frac{1}{2^k} \cdot e^{k\!\left(1 - \frac{n-k}{2n}\right)}.
	\end{align*}
	
	Поскольку $\frac{n-k}{2n} \geq \frac{1}{2}$ при $k \leq \frac{n}{2}$ и выражение достигает максимума при $k = 0$:
	\begin{align*}
		q_k
		\leq \frac{1}{2^k} \cdot e^{k\!\left(1 - \frac{1}{2}\right)}
