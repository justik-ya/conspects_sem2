...
\begin{Remark}{}{}
	Множество допустимых потоков ограничено и замкнуто, а целевая функция линейна. Следовательно, максимум существует.
\end{Remark}
...
\begin{proof}
	...
	\begin{enumerate}[label=\arabic*)]
		...
		\]
	\end{enumerate}
	На всех остальных рёбрах поток не меняется. Выбор $\delta \le c_f(v_i,v_{i+1})$
	гарантирует, что ограничения пропускной способности сохраняются, а закон сохранения потока нарушиться не может, поскольку изменение на каждом промежуточном узле компенсируется входящим и исходящим изменением одинаковой величины.
\end{proof}

\subsection{Алгоритм Форда--Фалкерсона}

\begin{Statement}{}{}
	Алгоритм Форда--Фалкерсона строит поток итеративно:
	\begin{enumerate}[label=\arabic*)]
		\item положить $f\equiv 0$;
		\item построить остаточную сеть $G_f$;
		\item найти в $G_f$ путь из $s$ в $t$;
		\item если такого пути нет, остановиться;
		\item если путь есть, увеличить поток вдоль него на величину $\delta(P)$ и перейти к следующей итерации.
	\end{enumerate}
\end{Statement}
...
\begin{Definition}{}{}
	...
	такое, что $S\cap T=\varnothing,\qquad s\in S,\qquad t\in T.$
\end{Definition}
