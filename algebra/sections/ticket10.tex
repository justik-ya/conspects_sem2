...
% (На доске часть записей была закрыта плакатом; по контексту восстановлено:
%  формула $\mathcal{B}(v,w) = [v]_E^T [\mathcal{B}]_E [w]_E$ применялась в следующем шаге.)
...
\textit{Решение.} Так как $\mathrm{char}\,\mathbb{F}_5 = 5 \neq 2$, теорема Лагранжа применима: любая симметрическая форма приводится к диагональному виду $\mathrm{diag}(d_1,\ldots,d_r,0,\ldots,0)$, $r = \mathrm{rk}\,\mathcal{B}$.

При замене $e_i \mapsto \gamma_i e_i$ коэффициент $d_i$ заменяется на $\gamma_i^2 d_i$.
Таким образом, два ненулевых значения $a, b \in \mathbb{F}_5^*$ определяют эквивалентные формы тогда и только тогда, когда $a/b \in (\mathbb{F}_5^*)^2$.

Вычислим квадраты в $\mathbb{F}_5$:
\[
1^2 = 1,\quad 2^2 = 4,\quad 3^2 = 4,\quad 4^2 = 1,
\]
откуда $(\mathbb{F}_5^*)^2 = \{1, 4\}$.
Группа квадратных классов $\mathbb{F}_5^*/(\mathbb{F}_5^*)^2 = \{[1], [2]\}$, где $[1] = \{1,4\}$ и $[2] = \{2,3\}$.

Каждый ненулевой диагональный элемент нормируется к представителю $1$ или $2$.
Форма ранга $r$ на $n$-мерном пространстве однозначно задаётся парой $(r,\,\delta)$:
\[
r \in \{0,1,\ldots,n\}, \qquad
\delta = [d_1\cdots d_r] \in \mathbb{F}_5^*/(\mathbb{F}_5^*)^2 \quad (r \geq 1).
\]
Для $r = 0$ класс один (нулевая форма).

Полный список представителей классов:
\begin{itemize}
	\item $r = 0$: $\mathrm{diag}(0,\ldots,0)$.
	\item $r \geq 1$, $\delta = [1]$: $\mathrm{diag}(\underbrace{1,\ldots,1}_{r},0,\ldots,0)$.
	\item $r \geq 1$, $\delta = [2]$: $\mathrm{diag}(2,\underbrace{1,\ldots,1}_{r-1},0,\ldots,0)$.
\end{itemize}

\begin{Statement}{}{}
	На $n$-мерном пространстве над $\mathbb{F}_5$ существует ровно $2n+1$ классов эквивалентности симметрических билинейных форм.
\end{Statement}
