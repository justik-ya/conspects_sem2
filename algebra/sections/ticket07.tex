...
\begin{Definition}{Фактормножество и факторгруппа}{def:quotient-set}
	Пусть $G$ --- группа, $H \triangleleft G$. Фактор\-множество $G/H$ определяется как множество левых смежных классов по $H$:
	\[
	G/H := \{gH \mid g \in G\}.
	\]
...
\begin{Definition}{Гомоморфизм групп}{def:hom}
	Пусть $G$ и $G'$ --- группы. Отображение
	\[
	\varphi\colon G \to G'
	\]
...
\begin{Theorem}{Индуцированный гомоморфизм}{induced-hom}
	...
	\varphi\colon G\to G'.
	\]
	Пусть $H\triangleleft G$, причём $H\subseteq \Ker\varphi$. Пусть также $H'\le G'$ и $\Im\varphi\subseteq H'$. Тогда существует единственный гомоморфизм
	\[
...
\begin{proof}
	Сначала докажем корректность. Если $g_1H=g_2H$, то $g_2=g_1h$ для некоторого $h\in H$. Так как $H\subseteq \Ker\varphi$, имеем $\varphi(h)=e$, и потому
	\[
	...
	Далее,
	\[
	\widetilde\varphi\bigl((g_1H)(g_2H)\bigr)=\widetilde\varphi(g_1g_2H)=\varphi(g_1g_2)=\varphi(g_1)\varphi(g_2)=\widetilde\varphi(g_1H)\widetilde\varphi(g_2H).
	\]
	Следовательно, $\widetilde\varphi$ --- гомоморфизм.
	
	Наконец, для любого $g\in G$
	\[
	(i_{H'}\circ \widetilde\varphi\circ \pi_H)(g)=i_{H'}\bigl(\widetilde\varphi(gH)\bigr)=i_{H'}(\varphi(g))=\varphi(g).
	\]
	Значит, диаграмма коммутирует.
	
	Единственность очевидна: если $\psi\colon G/H\to H'$ и $\varphi=i_{H'}\circ\psi\circ\pi_H$, то для любого $gH\in G/H$
	\[
	\psi(gH)=\psi(\pi_H(g))=\varphi(g)=\widetilde\varphi(gH).
	\]
	Следовательно, $\psi=\widetilde\varphi$.
\end{proof}
...
\begin{Theorem}{Теорема соответствия}{thm:correspondence}
	...
	\[
	\alpha\colon \{P\le G\mid H\subseteq P\}\to \{Q\le G/H\}, \qquad \alpha(P)=P/H,
	\]
	\[
...
\begin{proof}
	...
	Если $P\le G$ и $H\subseteq P$, то $P/H$ является подгруппой группы $G/H$, поскольку это образ подгруппы $P$ при гомоморфизме $\pi_H|_P\colon P\to G/H$.
	...
	\[
	(\beta\circ\alpha)(P)=\pi_H^{-1}(P/H)=\{g\in G\mid gH\in P/H\}.
	\]
	Но условие $gH\in P/H$ означает, что существует $p\in P$ такое, что $gH=pH$. Тогда $p^{-1}g\in H\subseteq P$, значит, $g\in P$. Итак,
	\[
	\pi_H^{-1}(P/H)=P.
	\]
...
\begin{Statement}{нормальность в факторгруппе}{normality-correspondence}
	...
	\[
	P/H \triangleleft G/H \Longleftrightarrow P \triangleleft G.
	\]
\end{Statement}
...
\begin{proof}
	...
	если $P/H\triangleleft G/H$, то
	\[
	P=\pi_H^{-1}(P/H)\triangleleft G.
	\]
	
	Обратно, пусть $P\triangleleft G$. Рассмотрим ограничение проекции на $P$. Тогда $P/H$ --- подгруппа в $G/H$ по теореме соответствия. Для любого $gH\in G/H$ и любого $pH\in P/H$ имеем
	\[
	(gH)(pH)(gH)^{-1}=(gpg^{-1})H.
	\]
	...
	(gpg^{-1})H\in P/H.
	\]
	Следовательно, $P/H\triangleleft G/H$.
\end{proof}
...
\begin{proof}
	Пункты 1) и 2) следуют соответственно из включения $H\subseteq P\subseteq G$ и утверждения~\ref{stmt:normality-correspondence}.
	
	Определим отображение
	\[
	\Psi\colon G/H \to G/P, \qquad \Psi(gH):=gP.
	\]
	Проверим корректность. Если $g_1H=g_2H$, то $g_2=g_1h$ для некоторого $h\in H\subseteq P$, следовательно, $g_1P=g_2P.$
	Значит, $\Psi$ корректно определено.
	
	Это гомоморфизм, поскольку
	\[
	\Psi\bigl((g_1H)(g_2H)\bigr)=\Psi(g_1g_2H)=g_1g_2P=(g_1P)(g_2P)=\Psi(g_1H)\Psi(g_2H).
	\]
	Он сюръективен, так как любой класс $gP\in G/P$ имеет прообраз $gH$.
	
	Ядро этого гомоморфизма равно
	\[
	\Ker\Psi=\{gH\in G/H\mid gP=P\}=\{gH\in G/H\mid g\in P\}=P/H.
	\]
	По первой теореме об изоморфизме получаем $(G/H)/(P/H)\cong G/P.$
\end{proof}
