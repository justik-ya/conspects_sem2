...
\begin{Definition}{Моменты случайной величины}{}
	...
	\begin{itemize}
		...
		\mu'_k = \mathbb{E}[\xi^k].
		\]
		При $k=1$ начальный момент $\mu'_1 = \mathbb{E}[\xi]$ --  \textit{математическое ожидание}.
		
		\item \textbf{Центральный момент} порядка $k$:
		\[
		\mu_k = \mathbb{E}\!\left[(\xi - \mathbb{E}[\xi])^k\right].
		\]
		По определению $\mu_1 = 0$, а величина $\mu_2 = \mathbb{E}[\xi - \mathbb{E}(\xi)]^{2}$ -- \textit{дисперсия}.
	\end{itemize}
\end{Definition}
...
\begin{Remark}{}{}
	\textbf{Алгебраическая интерпретация.} В пространстве случайных величин наборы $\{1, \xi, \xi^2, \ldots, \xi^k\}$ и $\{1, (\xi-E\xi), (\xi-E\xi)^2, \ldots, (\xi-E\xi)^k\}$ являются базисами одной и той же линейной оболочки — пространства многочленов от $\xi$ степени не выше $k$. 
	
	\textbf{Итог.} С операторной точки зрения это означает, что наборы моментов $\{\mu'_i\}_{i=1}^k$ и $\{\mu_i\}_{i=1}^k$ содержат \emph{идентичную информацию} о распределении: переход между ними является невырожденным полиномиальным преобразованием.
\end{Remark}
